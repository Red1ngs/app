...
```
.                                 ← цей репо (монорепо, лише оркестрація +
│                                    спільна документація)
├── docker-compose.yml       ← core-service + account-service + day-service
│                                + telegram-service + redis, усі разом,
│                                в одній мережі
├── .env.example
├── ARCHITECTURE.md, ADMIN_GUIDE.md
│
├── core-service/            ← бізнес-ядро (колишній корінь репо, колишній "app")
│   ├── Dockerfile
│   ├── pyproject.toml, uv.lock
│   ├── main.py, app.yaml
│   └── src/                 (scheduler, professions, БД, RPC-сервер)
│
├── day_service/              ← сиблінг-директорія В ЦЬОМУ репо, свій образ
├── telegram_service/         ← сиблінг-директорія В ЦЬОМУ репо, свій образ
```

Кожен із трьох (`core-service/`, `day_service/`, `telegram_service/`) —
самодостатня директорія зі своїм `Dockerfile`/`pyproject.toml` і власним
build-контекстом у `docker-compose.yml` (`context: ./core-service` тощо).
Корінь репо більше не бере участі в жодному Docker build — там лишились
тільки `docker-compose.yml` і спільна документація.

```
../account-service/          ← ОКРЕМИЙ репо-сиблінг (не тут), окремий
│                                образ, окрема БД. Спільний для core-service і
...
`SchedulerService`, `EventDrivenScheduler`, `StartupManager` — `Account`
тепер будується як `Account(account_id, app_cfg, repo)` — БЕЗ auth/network:
жодного email/password/proxy в конструкторі більше немає (див. нижче,
чому).

## Що змінилось у `core_account.py`

`Account` більше НЕ тримає email/password/proxy в жодному вигляді — ні
в конструкторі, ні як поле. `Account.connect()` тепер лише:
1. `account_client.connect(account_id)` — просить account-service підняти
   сесію за вже відомим `account_id` (без жодного пароля з нашого боку).
2. Створює легкий `BotSession`-фасад локально (без HTTP/socket) з отриманим
   `user_id`/`user_name`.

`account_client.register(...)` (передача email/password/proxy на
account-service) винесений з `Account.connect()` в
`SchedulerService.add_account()` і викликається РІВНО ОДИН РАЗ — коли
адміністратор додає новий акаунт (чи свідомо оновлює пароль). Це і є
відповідь на "де тепер зберігається пароль": ЛИШЕ на account-service, у
його власній БД. core-service — ні в `.env`, ні в своїй SQLite —
password/proxy більше не зберігає взагалі; локально лишається тільки
`account_id` (+ email, не секрет, для власного обліку/логів). При
рестарті `register_account()` для кожного акаунта з `repo.accounts`
просто створює порожній (без секретів) `Account` — пароль на цьому кроці
не потрібен, бо `connect()` його й не питає.
...
`MB_ACCOUNT_ID`) працює з ТИМИ САМИМИ акаунтами, що й `core-service`, а не своїми
...
- Шифрування password у власній БД account-service (зараз plaintext).
  Це вже ЄДИНЕ місце, де password взагалі зберігається — core-service
  більше не тримає копію ні в `.env`, ні в своїй SQLite (див. "Що
  змінилось у `core_account.py`" вище) — тож це найважливіше місце для
...
## Спрощення парсингу нових глав (manga_load) — прибрано розподіл по акаунтам

Раніше цикл «на сайті скінчились непрочитані глави → знайти нові» був
розбитий на два кроки з розподілом роботи між акаунтами:

1. `CatalogLoaderProfession` парсила одну сторінку каталогу
   (`CATALOG_PAGE_SIZE` = 30 манг) і збирала список нових `translit_name`.
2. `scheduler.dispatch_work(...)` рівномірно ділила цей список між
   УСІМА акаунтами, на яких активна профессія `manga_loader`, і кожен
   такий акаунт окремим `ask("update_mangas")` довантажував главі своєї
   частки манг. Лок на каталог знімався лише в `finally` останнього з
   цих запитів.

Це ускладнювало логіку (два незалежні профессії з непрямим зв'язком
через лок + широкомовний dispatch) без реальної потреби — довантаження
30 сторінок манги не настільки важке навантаження, щоб виправдовувати
розподіл між акаунтами.

**Що змінилось:**

- `dispatch_work()` і допоміжна `_split_evenly()` видалені зі
  `scheduler.py` — ніде більше не використовуються.
- `MangaLoaderProfession` більше не має інтенту `update_mangas` і не
  бере участі в автоматичному циклі. Лишився тільки `force_parse`
  (ручна команда оператора з Telegram-бота).
- `CatalogLoaderProfession` тепер сама довантажує глави щойно знайдених
  манг — на ТОМУ Ж акаунті, що спарсив каталог, без жодного dispatch.
  Спільна логіка «завантажити + розпарсити глави однієї манги» винесена
  у `fetch_chapter_rows()` (`manga_load/manga_loader.py`) — її
  використовують і `force_parse`, і catalog_loader.
- Глави зберігаються в БД не по одній манзі і не всі 30 одразу, а
  пачками по `CHAPTER_SAVE_BATCH_SIZE` = 5 манг (`manga_load/parsers.py`):
  обробили 5 — одразу `upsert_many()` — і далі. Якщо цикл перерветься
  посеред сторінки (акаунт відключився, мережева помилка) — уже
  оброблені пачки лишаються збереженими, а не губляться разом з усією
  сторінкою.
- `_on_chapters_exhausted` тепер один прямий `try/except/else/finally`:
  лок завжди знімається рівно один раз у `finally`, а не в кількох
  місцях умовно (як раніше з перевіркою `if dispatched == 0`).

Побічний ефект — увесь цикл «каталог → глави» вже й раніше йшов через
HTTP-запити з `priority="BACKGROUND"` (`bot_session.py`,
`fetch_manga_catalog`/`fetch_manga_page`/`fetch_manga_chapters`) —
найнижчий рівень черги запитів акаунта (`proxy_queue.Priority`). Тепер,
коли прогрес чекпоїниться кожні 5 манг, а не втрачається весь одразу
при перериванні, це фонове довантаження можна спокійно тримати саме на
цьому найнижчому пріоритеті — воно не блокує інтерактивні дії акаунта
і не вимагає резервувати під себе інші акаунти.

## Прибрано зберігання паролів в .env core-service

Раніше `SchedulerService` тримав `ACCOUNT_PASSWORD_<slug>` /
`ACCOUNT_PROXY_<slug>` у файлі `.env` поруч із собою (`_write_env` /
`_load_credentials` у `scheduler_service.py`) і перечитував їх звідти
при КОЖНІЙ побудові `Account` — і при hot-add, і при рестарті процесу
(`register_account()` для кожного акаунта з `repo.accounts`). Через це
`core-service` користувачу `appuser` знадобився `write`-доступ до `.env`
у контейнері — звідси й `PermissionError: [Errno 13] Permission denied:
'.env'` при спробі `add_account` (файл змонтований/створений з правами,
де `appuser` не може писати).

Корінь проблеми ширший за саму помилку прав: пароль/проксі не мали
жодної причини лежати ще й на боці core-service, коли account-service
вже й так зберігає їх у власній БД для логіну. Двійне джерело правди —
зайва складність і зайва поверхня витоку секретів.

**Що змінилось:**

- `scheduler_service.py`: видалені `_write_env`, `_remove_env`,
  `_save_credentials`, `_load_credentials`, `_erase_credentials`,
  `_pw_key`, `_proxy_key`, `_slug`, `_ENV_FILE` — увесь механізм `.env`
  для секретів прибрано без заміни на щось інше локальне.
- `core_account.py`: `Account.__init__` більше не приймає `auth`/
  `network` — конструктор тепер `Account(account_id, app_config, repo)`.
  `Account.connect()` більше НЕ викликає `account_client.register(...)` —
  лише `account_client.connect(account_id)` за вже відомим ідентифікатором.
- `SchedulerService.add_account()` — єдине місце, де
  `account_client.register(account_id, email, password, proxy)`
  викликається взагалі, і викликається РІВНО ОДИН РАЗ (коли адмін додає
  новий акаунт/свідомо оновлює пароль), напряму, без проміжного
  збереження де-небудь на core-service.
- `SchedulerService._register_impl()` (шлях рестарту, через
  `register_account()` з `main.py` для кожного акаунта з `repo.accounts`)
  більше не читає й не потребує пароль — просто будує порожній (без
  секретів) `Account`, а `connect()` сам попросить account-service
  підняти сесію за `account_id`.
- `SchedulerService.remove()` — прибраний виклик `_erase_credentials`,
  бо стирати вже нічого: локально секретів немає.
- `config/bot.py` видалений повністю — `AuthConfig`/`NetworkConfig`
  (і вже давно нічим не використовувані `ClientConfig`/`BotConfig`/
  `BaseHeaders`) стали мертвим кодом після цього рефакторингу.

Результат: `core-service` більше НІДЕ не зберігає password/proxy — ні в
`.env`, ні в SQLite, ні в пам'яті довше тривалості одного RPC-виклику.
Локально (`repo.accounts`) лишається тільки `account_id` + email (не
секрет) — рівно стільки, щоб знати, ЯКІ акаунти взагалі існують і кого
просити в account-service. Як побічний ефект — сам `PermissionError` на
`.env` більше не може повторитись, бо `core-service` туди більше нічого
не пише.
