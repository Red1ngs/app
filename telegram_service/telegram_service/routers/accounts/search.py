...
from telegram_service.core_client import AccountInfo, CoreServiceClient
...
async def cb_search_pick_filter(call: CallbackQuery, state: FSMContext, svc: CoreServiceClient) -> None:
...
async def fsm_search_query(message: Message, state: FSMContext, svc: CoreServiceClient) -> None:
...
async def cb_search_apply(call: CallbackQuery, svc: CoreServiceClient) -> None:
...
async def cb_group_pick(call: CallbackQuery, svc: CoreServiceClient) -> None:
...
async def cb_group_show(call: CallbackQuery, svc: CoreServiceClient) -> None:
    parts = call.data.split(":", 4)  # type: ignore[union-attr]
    if len(parts) < 5:
        await call.answer("❌ Помилка даних", show_alert=True)
        return
