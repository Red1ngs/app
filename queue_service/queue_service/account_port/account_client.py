"""
queue_service/account_port/account_client.py — клієнт до account-service (порт).

Ідентична копія src/core/account_client.py з бізнес-застосунку (app) —
файл самодостатній (лише httpx), тому переноситься між сервісами копіюванням.
Якщо колись знадобиться — винести в окремий pip-пакет спільний для всіх
сервісів; поки що це свідомий copy-paste, щоб сервіси лишались незалежними
один від одного (спільна залежність тільки — account-service як HTTP API).
